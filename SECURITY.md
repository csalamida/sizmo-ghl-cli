...
|-----------|---------------|
| **The PIT is read from stdin or env only — never argv.** There is no `--pit` flag, so your token never lands in shell history, `ps`, or process args. | `grep -rn "'--pit'" lib/ commands/` — you'll find **nothing**: no bare `--pit` literal exists in the source. (The flags that do exist — `--pit-stdin` and `--pit-env` — won't appear here because `'--pit-stdin'` has no closing apostrophe right after "pit".) To confirm those two flags exist: `grep -rn "pit-stdin\|pit-env" lib/ commands/`. |
