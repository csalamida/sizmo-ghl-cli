...
# findings — 2026-08-14 — test-coverage

## Enumeration: commands vs test files

All 36 `commands/*.mjs` files have at least one matching `test/commands/*.test.mjs`.

The 2026-07-16 backlog (`surveys.mjs`, `transactions.mjs`, `business.mjs`) is **fully addressed** — all three have substantive test files that pass:

| command | test file | tests |
|---------|-----------|-------|
| surveys.mjs | surveys.test.mjs | 18 — full branch set |
| transactions.mjs | transactions.test.mjs | 18 — full branch set |
| business.mjs | business.test.mjs | 36 — includes update verb added 2026-07-27 |

**Coverage is complete. No command is at zero.**
...
Test counts per file (bottom of distribution, sole-coverage files only):

```
 8  init.test.mjs
 9  diff.test.mjs    ← chosen: most uncovered branches per line of command logic
 9  doctor.test.mjs
```

Supplementary files (`sync-blocked`, `list-blocked`, `triage-concurrency`) excluded — they are intentionally narrow, paired with a primary test file.

**Chose `diff.mjs`** — uncovered branches:

| branch | covered before? |
|--------|----------------|
| `loadDoc`: invalid JSON → EXIT.USAGE | NO |
| `loadDoc`: valid JSON, no `specVersion` → EXIT.USAGE | NO |
| `diffGroup`: B-side non-array (only A-side tested) | NO |
| `diffGroup`: blocked group with `httpCode` → "API error X" reason | NO |
| `diffGroup`: non-array without `.blocked` → "unavailable" | NO |
| `keyOf` name fallback (items without `id`) | NO |
| Human card: `identical:true` early-return → "identical — no differences" | NO |
| Human card: `!r.comparable` loop branch → shows group label + reason | NO |

## Changes made

**File:** `test/commands/diff.test.mjs`
**Tests added:** 8 (9 → 17)

| test | branch covered |
|------|---------------|
| `diff run: invalid JSON file → EXIT.USAGE` | `loadDoc` JSON.parse failure (line 84) |
| `diff run: valid JSON but no specVersion → EXIT.USAGE` | `loadDoc` specVersion null check (line 85) |
| `diffDocs: B-side group blocked → comparable:false, reason names side B` | `diffGroup` side='B' ternary (line 33-34) |
| `diffDocs: blocked group with httpCode → reason says "API error X"` | `blockedNote` httpCode branch (line 35) |
| `diffDocs: non-array group with no blocked property → "unavailable"` | `marker?.blocked` falsy path (line 36) |
| `diffDocs: items without id keyed by name — add/remove/change all resolve` | `keyOf` name fallback (line 19) |
| `diff run: identical docs (file vs file) → card prints "identical — no differences"` | card `result.identical` early-return (line 107) |
| `diff run: not-comparable group → card shows group label + reason` | card `!r.comparable` branch (line 120) |

### Side-finding documented in test comment

`identical` is computed as `added===0 && removed===0 && changed===0` — `notComparable` does NOT flip it. A blocked group with no other differences produces `identical:true` and the card early-returns before showing the not-comparable reason. This is not a bug (the comparable data IS identical) but unintuitive. The not-comparable card test intentionally includes a real tag difference to force `identical:false`.
...
**+8 tests, 0 failures, 0 regressions.**

## Files changed

- `test/commands/diff.test.mjs` — 8 tests appended (lines 119–234 of the updated file)

## Files NOT changed

- All command source files — no bugs found requiring source changes.
