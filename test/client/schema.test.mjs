...
test('buildSchema emits command tree from metas', async () => {
  ...
  const s = await buildSchema(fakeRegistry, { OK:0, API:1, USAGE:2, AUTH:3, NOTFOUND:4, CONFIRM:5 });
  assert.equal(s.schemaVersion, 1);
  assert.equal(s.commands[0].name, 'snapshot');
  assert.deepEqual(s.exitCodes, { OK:0, API:1, USAGE:2, AUTH:3, NOTFOUND:4, CONFIRM:5 });
});
