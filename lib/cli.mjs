...
export async function route(argv, io = {}) {
  ...
  // pull global --profile + --json + --fresh/--no-cache + --concise + --fields
  let profile = null, json = false, fresh = false, concise = false, fields = null; const rest = [];
  ...
  try {
    if (registry[name]) {
      ...
      const ctx = buildCtx({ creds, globals: { json, tty, command: name, fresh, concise, fields } });
