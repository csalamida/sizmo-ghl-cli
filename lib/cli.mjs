...
import { probeLanes, buildLanes } from './diagnose.mjs';
import { buildCtx, CACHE_DIR } from './context.mjs';
...
export async function route(argv, io = {}) {
  ...
  try {
    ...
    // An UNEXPECTED error (a TypeError, a bad property read — anything not thrown deliberately)
    // used to emit only this bare line, with nothing at all on stdout. Verified 2026-07-30 by
    // inducing one on `sizmo crm tags --json`:
    //     exit=1 · STDOUT: (empty) · STDERR: Cannot read properties of null (reading 'boom')
    // So `--json` promised an envelope and delivered zero bytes. Anything parsing stdout got a
    // parse error on empty input, with no machine-readable cause — and a deliberate GhlError one
    // line above DOES get a JSON error object, so the two failure modes disagreed on their shape.
    // An internal error is still an error: it gets the same envelope, marked as internal so it is
    // not mistaken for an API or scope problem.
    if (json) {
      writeErr(JSON.stringify({
        error: `internal error: ${e?.message || 'unknown'}`,
        code: EXIT.API,
        internal: true,
        remediation: 'this is a bug in sizmo, not a problem with your account or token — please report it with the command you ran',
      }) + '\n');
    } else {
      writeErr(`internal error: ${e?.message || 'error'}\n  fix: this is a bug in sizmo, not your account — please report it with the command you ran\n`);
    }
    return EXIT.API;
  }
}
...
// The command list is DERIVED from the registry, never typed here. `sizmo --help` used to print
// a pointer to the schema command and no command names at all, so a first-time user had to run a
// command that emits a large JSON document to learn that `brief` exists. `init`, the onboarding
// verb, was missing from help entirely. Verified 2026-07-30.
//
// Wrapped rather than one-per-line: 33 commands one to a line buries the flags underneath them.
function commandLines(width = 76, indent = '    ') {
  const names = Object.keys(registry).sort();
  const lines = [];
  let cur = indent;
  for (const n of names) {
    if (cur.length + n.length + 1 > width) { lines.push(cur.trimEnd()); cur = indent; }
    cur += n + ' ';
  }
...
function helpText(v) { return `sizmo ${v} — GoHighLevel CLI (reads + confirm-gated writes; money never moves)
  ...
  setup:    sizmo init          # prints the GHL path + scopes, then the exact command to run
  commands:
${commandLines()}
  details:  sizmo help <command>   ·   sizmo schema  (machine-readable, includes subcommands)
  sizmo auth status · sizmo config list|use|set|rm|cache-clear
...
const COMMAND_EXAMPLES = {
  ...
  export:       ['sizmo export --out location.json', 'sizmo export > location.json', 'sizmo export --json'],
  diff:         ['sizmo diff location.json', 'sizmo diff before.json after.json', 'sizmo diff location.json --json'],
  ...
  appointment:  ['sizmo appointment list', 'sizmo appointment list --days 30', 'sizmo appointment book --calendar "Intro" --contact <id> --start 2026-07-01T10:00:00Z --confirm', 'sizmo appointment cancel <apptId> --confirm'],
  send:         ['sizmo send <contactId> --channel sms --message "Hi!"', 'sizmo send <contactId> --channel email --message "..." --confirm'],
  contact:      ['sizmo contact find "ana cruz"', 'sizmo contact find ana@example.com --limit 25', 'sizmo contact create --email a@b.co --name "Acme Co"', 'sizmo contact upsert --email a@b.co --name "Acme Co" --confirm', 'sizmo contact delete <contactId> --confirm'],
  calendar:     ['sizmo calendar create --name "Discovery Calls" --confirm', 'sizmo calendar create --name "Demos" --slot-min 45 --confirm', 'sizmo calendar create --name "Round Robin" --type round_robin --team-member uid1,uid2 --confirm', 'sizmo calendar delete <calendarId> --confirm'],
  ...
  invoice:      ['sizmo invoice list --status draft', 'sizmo invoice list --top 50', 'sizmo invoice draft --contact <id> --item "Consulting:5000" --currency PHP --confirm', 'sizmo invoice send <invoiceId> --confirm'],
  ask:          ['sizmo ask "who hasn\'t replied in 3 days"', 'sizmo ask "tag Ana Cruz as follow-up"', 'sizmo ask "move Website Package to Proposal Sent"', 'sizmo ask "send Marco a check-in SMS" --confirm'],
  list:         ['sizmo list', 'sizmo list calendars', 'sizmo list pipelines', 'sizmo list tags', 'sizmo list fields', 'sizmo list values', 'sizmo list users', 'sizmo list forms', 'sizmo list surveys', 'sizmo list products', 'sizmo list links', 'sizmo list businesses', 'sizmo list objects'],
  forms:        ['sizmo forms', 'sizmo forms <formId>', 'sizmo forms <formId> --top 50', 'sizmo forms --json'],
  surveys:      ['sizmo surveys', 'sizmo surveys <surveyId>', 'sizmo surveys <surveyId> --top 50'],
  business:     ['sizmo business list', 'sizmo business create --name "Acme Corp" --website "https://acme.com" --confirm', 'sizmo business delete <id> --confirm'],
  transactions: ['sizmo transactions', 'sizmo transactions --top 50', 'sizmo transactions --type subscription', 'sizmo transactions --json'],
...
async function routerVerb(cmd, args, io) {
  ...
  if (cmd === 'auth') {
    if (verb === 'check') {
      ...
      const creds = resolve(profile);
      if (!creds.pit) return die('no credentials found', EXIT.AUTH, 'set GHL_PIT + GHL_LOCATION_ID, or: echo "pit-…" | sizmo config set --profile <name> --pit-stdin --loc <LOCATION_ID>   (both, or the profile cannot be used)');
      if (!creds.loc) return die('no location resolved', EXIT.AUTH, 'set GHL_LOCATION_ID, or attach one to the profile: sizmo config set --profile <name> --loc <LOCATION_ID>');

      const loc = creds.loc;

      if (!json) write(`auth check: probing ${buildLanes(loc).length} GoHighLevel API scopes...\n`);
  ...
  if (cmd === 'config') {
    ...
    if (verb === 'set') {
      ...
      if (flag('--ai-key')) p.aiKey = flag('--ai-key');
      if (flag('--ai-provider')) p.aiProvider = flag('--ai-provider');
    ...
    // cache-clear: delete the on-disk read cache. Cached response bodies contain contact PII
    // (names, emails, phone numbers) and there was previously no way for a user to remove them —
    // expired entries were ignored by `get` but left on disk indefinitely.
    if (verb === 'cache-clear') {
      const { removed } = makeCache({ dir: CACHE_DIR, ttlMs: 0 }).clear();
      if (json) { write(JSON.stringify({ schemaVersion: 1, cacheDir: CACHE_DIR, removed }) + '\n'); return EXIT.OK; }
      write(`cleared ${removed} cached response${removed === 1 ? '' : 's'} from ${CACHE_DIR}\n`);
      return EXIT.OK;
    }

    return die('usage: sizmo config list|use <name>|set --profile <name> ...|rm <name>|cache-clear', EXIT.USAGE);
  }
...
async function apiVerb(args, { profile, json, write, writeErr }) {
  ...
  if (!path || !path.startsWith('/'))
    return die('usage: sizmo api </path?query> [--paginate] [--max-pages N] [--no-loc]', EXIT.USAGE, 'example: sizmo api "/contacts/?limit=5"');

  const creds = resolve(profile);
  if (!creds.pit)
    return die('no PIT available', EXIT.AUTH, 'set GHL_PIT + GHL_LOCATION_ID, or: echo "pit-…" | sizmo config set --profile <name> --pit-stdin --loc <LOCATION_ID>   (both, or the profile cannot be used)');
  ...
  // auto-fill locationId when absent — most top-level entity list endpoints need it.
  // Some sub-resource endpoints (e.g. /contacts/:id/notes) reject ANY unrecognized query
  // param with a 422, including an auto-injected locationId they never asked for — verified
  // live 2026-07-05. --no-loc skips the injection for exactly that case.
...
function openVerb(args, { profile, json, write, writeErr }, spawnFn = spawn) {
  ...
  // REFUSE extra positionals rather than silently using the first.
  //
  // `open` takes an id and infers the kind from --opp, but the rest of the CLI reads
  // `sizmo <command> <kind> <id>` (`sizmo list contacts`, `sizmo business update <id>`), so
  // `sizmo open contact cid-1` is a very plausible mistyping. It used to take "contact" as the id,
  // ignore "cid-1", and emit a well-formed URL to a record that does not exist — with exit 0 and no
  // warning. The user clicks a link that looks right and lands on a 404 inside GoHighLevel.
  // A silently wrong answer is worse than a refusal.
  if (positionals.length > 1) {
    return die(
      `sizmo open takes exactly one id — got ${positionals.length}: ${positionals.join(', ')}`,
      EXIT.USAGE,
      `the kind comes from a flag, not a word: sizmo open ${positionals[positionals.length - 1]} [--opp]`);
  }
  const creds = resolve(profile);
  if (!creds.loc) return die('no location resolved', EXIT.AUTH, 'set GHL_LOCATION_ID, or attach one to the profile: sizmo config set --profile <name> --loc <LOCATION_ID>');
