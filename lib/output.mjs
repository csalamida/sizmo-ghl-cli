...
// Non-objects (primitives, null) are returned as-is — and so are ARRAYS.
//
// Arrays are not projectable: a field list names KEYS, and an array has indices. `typeof []` is
// 'object', so without the explicit guard an array item was rebuilt as {} and a nested list silently
// became an empty object. Found by a mutation on projectPayload's guard, which turned out to be
// masking this rather than fixing it.
...
function projectPayload(data, fields) {
  ...
  for (const [k, v] of Object.entries(result)) {
    ...
    // Did the projection find anything anywhere in this array? If not, the field list does not apply
    // here, so hand back the original rather than a row of empties.
    //
    // A first draft also carried a `wasObjects` guard meant to exempt arrays of primitives. It was
    // redundant — project() already returns non-objects unchanged, so both branches agree for them —
    // and it was actively WRONG for an array OF ARRAYS, which it converted to [{}]. A mutation that
    // deleted it produced better output than the code it was part of. Removed; the real fix went
    // into project() instead.
    const anyKept = projected.some(x => x && typeof x === 'object' && Object.keys(x).length > 0);
    result[k] = anyKept ? projected : v;
  }
  return result;
}
