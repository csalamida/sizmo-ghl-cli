...
// A short, non-reversible fingerprint of the PIT, used to scope cache entries to the token that
// fetched them. NEVER the token itself: the fingerprint becomes part of a cache key, and cache keys
// are hashed into filenames, so a raw PIT must not enter that path.
const pitPrint = (pit) => createHash('sha256').update(String(pit ?? '')).digest('hex').slice(0, 16);
...
export function makeHttp({ pit, base = DEFAULT_BASE, version = '2021-07-28',
  ...
  async function get(path, { query, version: v = version } = {}) {
    const url = new URL(base + path);
    if (query) for (const [k, val] of Object.entries(query)) if (val != null) url.searchParams.set(k, String(val));
    // Cache key = TOKEN FINGERPRINT + resolved URL.
    //
    // The URL alone was not enough, and the old comment here claimed otherwise ("includes
    // locationId param → no cross-profile bleed"). That reasoning only holds if each PIT maps to a
    // distinct location. Two profiles pointing at the SAME location with DIFFERENT tokens — an
    // agency PIT and a client-supplied PIT, or a rotated token with fewer scopes — collided on the
    // same key, and the cache dir is one shared ~/.config/sizmo/cache for every profile.
    //
    // Verified 2026-07-28 with two tokens on one location:
    //     profile A (full scopes)        code=200 contacts=1
    //     profile B (NO contacts scope)  code=200 contacts=1  serverAsked=0
    // B read A's contact data — including an email address — with a token that cannot read
    // contacts, and B's own server was never contacted. SECURITY.md's central claim is "the PIT
    // scope is the gate"; a shared cache key bypassed that gate for the full 60s TTL.
    //
    // The fingerprint is a truncated sha256, never the PIT — see pitPrint.
    const cacheKey = pitPrint(pit) + '|' + url.toString();
    if (cache && !fresh) {
