// lib/confirm.mjs — exit-4 confirmation envelope. The universal agent-safety gate.
//
...
//   neither         → prints/emits the envelope, exits 4 (CONFIRM), never executes
//
// The agent pattern:
//   1. Agent runs command WITHOUT --confirm → sees exit 4 + confirmCommand
...
export function requireConfirm({ command, changes, rerunCommand }, ctx) {
  ...
  // No --confirm → confirmation-required envelope (exit 4)
