...
export function saveProfiles(db) {
  ...
  // 0700, not the default 0755. The FILE is 0600 so the PIT itself is never exposed, but a
  // world-readable directory still leaks metadata — that a sizmo profile store exists, its size,
  // when it last changed. Same convention ssh enforces on ~/.ssh. Only applies on creation; an
  // existing directory keeps its mode.
  mkdirSync(dir, { recursive: true, mode: 0o700 });

  // Atomic write: create the temp at 0600, then rename atomically. Prevents partial-write
  // corruption and any window where a cleartext PIT is world-readable.
  //
  // `flag: 'wx'` and the pre-unlink are both load-bearing, and the reason is a real footgun:
  // writeFileSync's `mode` is ONLY honoured when it CREATES the file. Verified 2026-07-27 —
  // writing 0600 over an existing 0644 file leaves it 0644. So if a temp survived (a SIGKILL or
  // power loss skips the `finally` below) and the pid was later reused, the next save would write
  // the PIT into that file WITHOUT applying 0600, silently. SECURITY.md promises "no window where
  // it's world-readable"; that promise now holds unconditionally rather than only on the happy
  // path.
  //
  // If another process wins a race to create the temp between the unlink and the write, 'wx'
  // throws EEXIST — refusing is correct. Writing a credential into a file someone else created,
  // with their permissions, is the exact outcome worth failing to avoid.
  const tmp = PROFILES + '.tmp.' + process.pid;
  try {
    try { rmSync(tmp, { force: true }); } catch { /* fall through — 'wx' catches anything left */ }
    writeFileSync(tmp, JSON.stringify(db, null, 2), { mode: 0o600, flag: 'wx' });
...
export function resolveCreds(env, profile) {
  ...
  const aiKey = env.SIZMO_AI_KEY || profile?.aiKey || null;
  const aiProvider = env.SIZMO_AI_PROVIDER || profile?.aiProvider || 'anthropic';
  return { pit, loc, tz, currency, source, aiKey, aiProvider };
}
